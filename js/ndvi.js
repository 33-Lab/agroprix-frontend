/*! AgroPrix ndvi.js - generated from ndvi.js.src on 2026-06-27 - DO NOT EDIT; edit the .src file and run `python build_js.py` */
...
function fetchBackendTimeseries(backendId){return apiFetch('/api/ndvi/parcelle/'+backendId+'/timeseries?limit=50').then(function(r){if(r.status===404)return{points:[],note:'Aucune serie NDVI reelle calculee pour cette parcelle'};if(r.status===401)throw new Error('auth');if(!r.ok)throw new Error('HTTP '+r.status);return r.json();});}
function fetchSentinelNDVI(backendId){return apiFetch('/api/ndvi/sentinel/parcelle/'+backendId+'?days=90&n_scenes=3',{method:'GET'}).then(function(r){if(r.status===401)throw new Error('auth');if(!r.ok)throw new Error('HTTP '+r.status);return r.json();});}
function analyzeSatellite(idx){var parcels=getParcels();var p=parcels[idx];if(!p)return;var btn=document.getElementById('sat-btn-'+idx);if(btn){btn.disabled=true;btn.textContent='Analyse satellite en cours (~10s)...';}
syncParcelToBackend(idx).then(function(backendId){return fetchSentinelNDVI(backendId);}).then(function(res){res=res||{};var best=res.best||null;var analysable=(res.analysable===true)&&best&&best.mean!==null&&best.mean!==undefined;if(!analysable){if(btn){btn.disabled=false;btn.innerHTML='<i data-lucide="satellite" class="lc"></i> Reessayer l\'analyse satellite';if(window.lucide)try{window.lucide.createIcons();}catch(e){}}
showSatelliteIssue(idx,res);return;}
var ps=getParcels();ps[idx].realNDVI={mean:best.mean,min:best.min,max:best.max,date:String(best.datetime||'').split('T')[0],cloud:best.cloud_cover,validFraction:best.valid_fraction,source:res.source||'sentinel-2'};ps[idx].realHistory=(res.stats||[]).filter(function(s){return s&&s.mean!==null&&s.mean!==undefined;}).map(function(s){return{date:String(s.datetime||'').split('T')[0],ndvi:Math.round((s.mean||0)*100)/100};}).reverse();ps[idx].realIssue=null;saveParcels(ps);render();}).catch(function(e){if(btn){btn.disabled=false;btn.innerHTML='<i data-lucide="satellite" class="lc"></i> Analyser au satellite (reel)';if(window.lucide)try{window.lucide.createIcons();}catch(ex){}}
if(String(e&&e.message)==='auth')alert('Connectez-vous pour lancer l\'analyse satellite reelle.');else alert('Analyse satellite indisponible : '+(e&&e.message?e.message:'erreur'));});}
function showSatelliteIssue(idx,res){var reasons=(res&&res.reasons)||[];var steps=(res&&res.next_steps)||[];var ps=getParcels();if(ps[idx]){ps[idx].realIssue={status:res.status||'low_quality',reasons:reasons,next_steps:steps,at:String(res.computed_at||'').split('T')[0]};saveParcels(ps);render();}
var msg=(reasons.length?reasons.join('\n'):'Analyse satellite indisponible pour le moment.')
+(steps.length?'\n\nQue faire :\n• '+steps.join('\n• '):'');alert(msg);}
var THRESHOLDS={excellent:0.7,good:0.5,moderate:0.3,stress:0.2};function getNDVIStatus(ndvi){if(ndvi>=THRESHOLDS.excellent)return{label:'Excellente',color:'#2D6A4F',bg:'#D8F3DC',emoji:'<span style="color:#2D6A4F;font-size:1em;">●</span>',advice:'Vegetation dense et saine. Conditions optimales.'};if(ndvi>=THRESHOLDS.good)return{label:'Bonne',color:'#40916C',bg:'#E8F5E9',emoji:'<span style="color:#FBBF24;font-size:1em;">●</span>',advice:'Vegetation en bon etat. Surveillez l\'irrigation.'};if(ndvi>=THRESHOLDS.moderate)return{label:'Moderee',color:'#E8862A',bg:'#FFF8E1',emoji:'<span style="color:#F97316;font-size:1em;">●</span>',advice:'Stress vegetal detecte. Verifiez l\'eau et les intrants.'};if(ndvi>=THRESHOLDS.stress)return{label:'Stress',color:'#e76f51',bg:'#FFF3E0',emoji:'<span style="color:#EF4444;font-size:1em;">●</span>',advice:'Stress severe ! Risque de perte de recolte. Action urgente.'};return{label:'Critique',color:'#c0392b',bg:'#FFEBEE',emoji:'<span style="color:#991B1B;font-size:1em;">●</span>',advice:'Parcelle en danger critique. Sol nu ou vegetation morte.'};}
function getParcels(){try{return JSON.parse(localStorage.getItem(PARCELS_KEY)||'[]');}catch(e){return[];}}
function saveParcels(parcels){localStorage.setItem(PARCELS_KEY,JSON.stringify(parcels));}
function addParcel(parcel){var parcels=getParcels();parcel.id='P'+Date.now();parcel.createdAt=new Date().toISOString();parcel.realHistory=[];parcels.push(parcel);saveParcels(parcels);return parcel;}
function ndviSeries(p){return(p&&p.realHistory&&p.realHistory.length)?p.realHistory:[];}
function ndviLatest(p){if(p&&p.realNDVI&&typeof p.realNDVI.mean==='number'){return{ndvi:Math.round((p.realNDVI.mean||0)*100)/100,date:p.realNDVI.date||'',cloud:p.realNDVI.cloud,validFraction:p.realNDVI.validFraction};}
return null;}
...
+'<div style="font-size:22px;color:#2D6A4F;">●</div><div style="font-size:10px;font-weight:600;color:#2D6A4F;">Saine</div>'
...
+'<div style="font-size:22px;color:#F97316;">●</div><div style="font-size:10px;font-weight:600;color:#E8862A;">Moderee</div>'
...
+'<div style="font-size:22px;color:#FBBF24;">●</div><div style="font-size:10px;font-weight:600;color:#40916C;">Bonne</div>'
...
+'<div style="font-size:22px;color:#EF4444;">●</div><div style="font-size:10px;font-weight:600;color:#e76f51;">Stress</div>'
+'<div style="font-size:9px;color:#666;">NDVI < 0.3</div></div>'
+'</div></div>';}
function renderDashboard(parcels){var totalNDVI=0;var analyzedCount=0;var alertCount=0;parcels.forEach(function(p){var l=ndviLatest(p);if(!l)return;analyzedCount++;totalNDVI+=l.ndvi;if(l.ndvi<THRESHOLDS.good)alertCount++;});var hasData=analyzedCount>0;var avgNDVI=hasData?(totalNDVI/analyzedCount).toFixed(2):null;var avgStatus=hasData?getNDVIStatus(avgNDVI):{color:'#94a3b8',emoji:'',label:'Non analysé'};var html='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px;">';html+='<div class="card" style="padding:12px;text-align:center;border-top:3px solid '+avgStatus.color+';">'
+'<div style="font-size:10px;color:var(--text-light);">NDVI moyen</div>'
+'<div style="font-size:24px;font-weight:800;color:'+avgStatus.color+';">'+(hasData?avgNDVI:'—')+'</div>'
+'<div style="font-size:10px;font-weight:600;color:'+avgStatus.color+';">'+(hasData?(avgStatus.emoji+' '+avgStatus.label):(analyzedCount+'/'+parcels.length+' analysées'))+'</div>'
+'</div>';html+='<div class="card" style="padding:12px;text-align:center;border-top:3px solid var(--primary);">'
...
+'</div>';var alertColor=alertCount>0?'#e76f51':'#2D6A4F';html+='<div class="card" style="padding:12px;text-align:center;border-top:3px solid '+alertColor+';">'
...
+'</div>';html+='</div>';html+='<div class="card" style="padding:12px;margin-bottom:16px;background:linear-gradient(135deg,#D8F3DC,#B7E4C7);border:1px solid #2D6A4F33;">'
+'<div style="display:flex;align-items:center;gap:10px;">'
+'<span style="font-size:28px;color:#1B4332;"><i data-lucide="shield" class="lc"></i></span>'
+'<div>'
+'<div style="font-size:12px;font-weight:700;color:#1B4332;">Apercu (phase 2) \u2014 Assurance parametrique prevue</div>'
+'<div style="font-size:11px;color:#2D6A4F;">Principe envisage : si le NDVI chute sous un seuil, l\'indemnisation serait declenchee sans expert terrain. Cette feature n\'est pas encore active.</div>'
+'</div></div></div>';return html;}
function renderParcelsList(parcels){var html='<div class="card" style="padding:16px;margin-bottom:12px;">'
+'<div class="card-title"><span class="icon"><i data-lucide="map" class="lc"></i></span> Mes parcelles</div>';parcels.forEach(function(p,idx){var latest=ndviLatest(p);var hist=ndviSeries(p);var bg=latest?getNDVIStatus(latest.ndvi).bg:(p.realIssue?'#FEF3C7':'#F1F5F9');var headColor=latest?getNDVIStatus(latest.ndvi).color:(p.realIssue?'#92400E':'#64748b');html+='<div style="padding:12px;border-radius:10px;background:'+bg+';margin-bottom:10px;cursor:pointer;" '
...
+'</div>';if(latest){var status=getNDVIStatus(latest.ndvi);html+='<div style="text-align:right;">'
...
+renderSparkline(hist,120,30)
+'<div style="font-size:10px;color:#2D6A4F;font-weight:600;"><i data-lucide="satellite" class="lc"></i> Sentinel-2 reel · '+(latest.date||'—')+' · nuages '+Math.round(latest.cloud||0)+'%'+(latest.validFraction!=null?' · fiabilite '+Math.round(latest.validFraction*100)+'%':'')+'</div>'
+'</div>'
+'<div style="font-size:11px;color:'+status.color+';margin-top:6px;font-style:italic;"><i data-lucide="lightbulb" class="lc"></i> '+status.advice+'</div>';}else if(p.realIssue){var iss=p.realIssue;html+='<div style="text-align:right;">'
+'<div style="font-size:18px;font-weight:800;color:#92400E;">—</div>'
+'<div style="font-size:10px;font-weight:600;color:#92400E;"><span style="color:#E8862A;">●</span> Non analysé</div>'
+'</div></div>'
+'<div style="font-size:11px;color:#92400E;margin-top:8px;"><i data-lucide="cloud-off" class="lc"></i> '+((iss.reasons&&iss.reasons[0])||'Analyse satellite non disponible.')+'</div>'
+((iss.next_steps&&iss.next_steps.length)?'<div style="font-size:11px;color:#92400E;margin-top:6px;background:#fff;border-radius:8px;padding:8px;"><div style="font-weight:700;margin-bottom:4px;"><i data-lucide="info" class="lc"></i> Que faire pour obtenir l\'analyse :</div><ul style="margin:0;padding-left:16px;">'+iss.next_steps.map(function(s){return'<li>'+s+'</li>';}).join('')+'</ul></div>':'');}else{html+='<div style="text-align:right;">'
...
html+='<button id="sat-btn-'+idx+'" data-action="ndvi-analyze-satellite" data-idx="'+idx+'" style="margin-top:8px;width:100%;padding:8px;border:1px solid var(--primary);background:#fff;color:var(--primary);border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;"><i data-lucide="satellite" class="lc"></i> '+(latest?'Reactualiser le satellite (reel)':(p.realIssue?'Reessayer l\'analyse satellite':'Analyser au satellite (reel)'))+'</button>'
...
+'</div></div>';html+='<div class="card" style="padding:16px;margin-bottom:16px;text-align:center;">'
...
+'</div>';});html+='</div>';}else{html+='<div style="font-size:12px;color:#94a3b8;text-align:center;padding:20px 8px;font-style:italic;">Aucune mesure satellite pour le moment. Lancez l\'analyse pour construire l\'historique NDVI.</div>';}
html+='</div>';html+='<div class="card" style="padding:12px;margin-bottom:16px;">'
+'<div class="card-title"><span class="icon"><i data-lucide="bar-chart-3" class="lc"></i></span> Echelle NDVI</div>'
...
+'</div></div>';html+='<div class="card" style="padding:16px;margin-bottom:16px;background:linear-gradient(135deg,#FFF8E1,#FFECB3);border:1px solid #E8862A33;">'
+'<div class="card-title" style="color:#b45309;"><span class="icon"><i data-lucide="shield" class="lc"></i></span> Assurance parametrique <span style="font-size:10px;background:#E8862A;color:#fff;padding:2px 6px;border-radius:4px;margin-left:6px;">PHASE 2</span></div>'
+'<p style="font-size:12px;color:#92400e;margin-bottom:8px;">'
+'Apercu : si le NDVI de cette parcelle descendait en dessous de <b>0.3</b> pendant 2 mesures consecutives, '
+'la parcelle serait potentiellement eligible a une indemnisation. Feature prevue en phase 2 (partenariat assureur a contractualiser).</p>'
...
+'<div style="background:linear-gradient(180deg,#fff,#FAFCFB);padding:8px;border-radius:8px;"><div style="font-size:10px;color:#666;">Statut actuel</div><div style="font-weight:700;color:'+(!latest?'#94a3b8':(latest.ndvi>=0.3?'#2D6A4F':'#e76f51'))+';">'+(!latest?'Non analysée':(latest.ndvi>=0.3?'Couvert':'ALERTE'))+'</div></div>'
+'</div></div>';html+='<div class="card" style="padding:16px;margin-bottom:16px;background:linear-gradient(135deg,#E0F2FE,#DBEAFE);border:1px solid #1E40AF33;">'
+'<div class="card-title" style="color:#1E40AF;"><span class="icon"><i data-lucide="satellite-dish" class="lc"></i></span> NDVI reel (Planet Insights)</div>'
+'<div id="ndviBackendStatus-'+idx+'" style="font-size:12px;color:#1E3A8A;margin:8px 0;">'
...
+'<select id="ndviCulture" style="width:100%;padding:10px;border:1px solid var(--border);border-radius:8px;font-family:inherit;">';cultures.forEach(function(c){html+='<option value="'+c+'">'+(AP.cultureNames[c]||c)+'</option>';});html+='</select></div>';html+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">'
...
+'style="width:100%;padding:10px;border:1px solid var(--border);border-radius:8px;font-family:inherit;box-sizing:border-box;">'
+'</div></div>';html+='<div class="form-group" style="margin-bottom:12px;">'
...
var parcel={name:name,culture:AP.cultureNames?(AP.cultureNames[culture]||culture):culture,cultureKey:culture,surface:surface,location:location,lat:lat,lon:lon};addParcel(parcel);if(AP.scoring&&AP.scoring.calculate)AP.scoring.calculate();render();}
function deleteParcel(idx){var parcels=getParcels();parcels.splice(idx,1);saveParcels(parcels);render();}
function init(){render();}
function syncToBackend(idx){var statusEl=document.getElementById('ndviBackendStatus-'+idx);if(statusEl)statusEl.innerHTML='<em>Synchronisation en cours...</em>';syncParcelToBackend(idx).then(function(backendId){if(statusEl)statusEl.innerHTML='<span style="color:#047857;">✓ Synchronise. Parcelle backend #'+backendId+'. Cliquez "Charger NDVI reel" pour voir la serie calculee.</span>';setTimeout(function(){showDetail(idx);},600);}).catch(function(err){if(statusEl)statusEl.innerHTML='<span style="color:#991B1B;">Erreur: '+(err.message||err)+'</span>';});}
function loadBackendTimeseries(idx){var parcels=getParcels();var p=parcels[idx];if(!p||!p.backendId)return;var seriesEl=document.getElementById('ndviBackendSeries-'+idx);if(seriesEl)seriesEl.innerHTML='<div style="padding:12px;opacity:0.6;font-size:12px;">Chargement timeseries backend...</div>';fetchBackendTimeseries(p.backendId).then(function(data){if(!seriesEl)return;var pts=(data&&data.points)||[];if(!pts.length){seriesEl.innerHTML='<div style="padding:12px;font-size:12px;color:#92400E;background:#FEF3C7;border-radius:8px;margin-top:8px;">'
+'<strong>Aucune serie NDVI reelle calculee pour cette parcelle.</strong><br>'
+'Pour generer une serie : soumettez un ordre Planet (POST /api/ndvi/order) sur des scenes PSScene, attendez state=success, puis POST /api/ndvi/order/{id}/compute. '
+'Cette action consomme du quota Planet.'
+'</div>';return;}
var rows=pts.map(function(pt){var d=(pt.date||'').substring(0,10);var mean=pt.mean!=null?pt.mean.toFixed(3):'—';var minV=pt.min!=null?pt.min.toFixed(2):'—';var maxV=pt.max!=null?pt.max.toFixed(2):'—';var pix=pt.pixels||0;return'<tr style="font-size:11px;border-bottom:1px solid #E5E7EB;">'
+'<td style="padding:4px 6px;">'+d+'</td>'
+'<td style="padding:4px 6px;text-align:right;font-weight:700;color:'+getNDVIStatus(pt.mean||0).color+';">'+mean+'</td>'
+'<td style="padding:4px 6px;text-align:right;color:#6B7280;">'+minV+' / '+maxV+'</td>'
+'<td style="padding:4px 6px;text-align:right;color:#9CA3AF;">'+pix.toLocaleString('fr-FR')+' px</td>'
+'</tr>';}).join('');var ordered=pts.slice().reverse();var sparkHistory=ordered.map(function(pt){return{date:(pt.date||'').substring(0,10),ndvi:pt.mean||0};});var spark=renderSparkline(sparkHistory,280,50);seriesEl.innerHTML='<div style="margin-top:10px;">'
+'<div style="text-align:center;margin-bottom:6px;">'+spark+'</div>'
+'<div style="font-size:11px;color:#6B7280;margin-bottom:4px;">'+pts.length+' mesure(s) reelle(s) \u2014 source: '+(data.source||'Planet')+'</div>'
+'<div style="max-height:240px;overflow-y:auto;"><table style="width:100%;border-collapse:collapse;">'
+'<thead><tr style="font-size:10px;color:#6B7280;border-bottom:2px solid #D1D5DB;">'
+'<th style="padding:4px 6px;text-align:left;">Date</th>'
+'<th style="padding:4px 6px;text-align:right;">NDVI moyen</th>'
