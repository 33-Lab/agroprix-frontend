...
+'</div></div>';}
function renderDashboard(parcels){var totalNDVI=0;var alertCount=0;parcels.forEach(function(p){var latest=p.ndviHistory&&p.ndviHistory.length>0?p.ndviHistory[p.ndviHistory.length-1].ndvi:0;totalNDVI+=latest;if(latest<THRESHOLDS.good)alertCount++;});var avgNDVI=parcels.length>0?(totalNDVI/parcels.length).toFixed(2):0;var avgStatus=getNDVIStatus(avgNDVI);var html='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px;">';html+='<div class="card" style="padding:12px;text-align:center;border-top:3px solid '+avgStatus.color+';">'
...
+'<div class="card-title"><span class="icon"><i data-lucide="map" class="lc"></i></span> Mes parcelles</div>';parcels.forEach(function(p,idx){var latest=p.ndviHistory&&p.ndviHistory.length>0?p.ndviHistory[p.ndviHistory.length-1]:{ndvi:0,date:''};var status=getNDVIStatus(latest.ndvi);var sparkline=renderSparkline(p.ndviHistory||[],120,30);html+='<div style="padding:12px;border-radius:10px;background:'+status.bg+';margin-bottom:10px;cursor:pointer;" '
...
+'</div></div>';html+='<button style="width:100%;padding:10px;border:1px solid #e76f51;color:#e76f51;background:linear-gradient(180deg,#fff,#FAFCFB);border-radius:8px;font-size:12px;cursor:pointer;" '
...
AP.ndvi={init:init,render:render,showAddForm:showAddForm,showDetail:showDetail,saveParcel:saveParcel,deleteParcel:deleteParcel,getGPS:getGPS,getParcels:getParcels};})(window.AgroPrix);