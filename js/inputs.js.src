...
(function(AP) {
  ...
  function loadCatalog() {
    ...
    return fetch('/data/inputs_catalog.json')
      .then(function(r) { return r.json(); })
      .then(function(data) { catalog = data; return data; })
      .catch(function() { console.warn('Could not load inputs_catalog.json'); return null; });
  }
  ...
  function render() {
    ...
    [
      { id: 'saisie', label: '📝 Saisir un prix', icon: '' },
  ...
  function renderHistorique() {
    ...
    // Full history list
    html += '<div class="card" style="padding:16px;">'
      + '<div class="card-title"><span class="icon">📋</span> Tous les prix saisis</div>';
    ...
    comparables.forEach(function(productId) {
      ...
      if (diffPct > 10) {
        html += '<div style="margin-top:8px;padding:8px;background:#D8F3DC;border-radius:8px;font-size:12px;color:#1B4332;">'
          + '💡 <strong>' + prod.name + '</strong> est <strong>' + diffPct + '% moins cher</strong> a ' + entries[0].market + ' qu\'a ' + entries[entries.length - 1].market + '.</div>';
      }
  ...
  function submitPrice() {
    ...
    if (successEl) {
      successEl.textContent = '✅ Prix enregistre ! ' + entry.productName + ' a ' + formatFCFA(price) + ' (' + entry.market + ')';
