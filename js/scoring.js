/*! AgroPrix scoring.js - generated from scoring.js.src on 2026-06-25 - DO NOT EDIT; edit the .src file and run `python build_js.py` */
...
var marketData=getMarketplaceData();if(marketData.offers>0)prodPts+=Math.min(marketData.offers*10,40);scores.production=Math.min(prodPts,200);var transPts=0;if(marketData.confirmedSales>0){transPts+=Math.min(marketData.confirmedSales*30,120);}
if(marketData.totalVolumeFCFA>0){if(marketData.totalVolumeFCFA>=10000000)transPts+=80;else if(marketData.totalVolumeFCFA>=5000000)transPts+=60;else if(marketData.totalVolumeFCFA>=1000000)transPts+=40;else transPts+=20;}
if(marketData.rating&&marketData.rating>=4)transPts+=50;else if(marketData.rating&&marketData.rating>=3)transPts+=30;scores.transactions=Math.min(transPts,250);var stabPts=0;if(profile.isCoop)stabPts+=40;if(profile.hasBusinessPlan)stabPts+=40;if(profile.farmerType==='sme')stabPts+=30;else if(profile.farmerType==='cooperative')stabPts+=25;else stabPts+=10;var searches=getFinancingSearches();if(searches.length>=3)stabPts+=20;else if(searches.length>=1)stabPts+=10;if(profile.crops&&profile.crops.length>0&&profile.farmSize>0&&profile.experience>0){stabPts+=20;}
scores.stability=Math.min(stabPts,150);var usagePts=0;var analysesCount=parseInt(localStorage.getItem(ANALYSES_KEY)||'0');if(analysesCount>=10)usagePts+=40;else if(analysesCount>=5)usagePts+=30;else if(analysesCount>=1)usagePts+=15;if(searches.length>=5)usagePts+=30;else if(searches.length>=2)usagePts+=20;else if(searches.length>=1)usagePts+=10;var inputPrices=JSON.parse(localStorage.getItem('agroprix_input_prices')||'[]');if(inputPrices.length>=5)usagePts+=30;else if(inputPrices.length>=1)usagePts+=15;scores.usage=Math.min(usagePts,100);var senPts=0;if(user&&user.cree_le){var created=new Date(user.cree_le);var now=new Date();var daysSince=Math.floor((now-created)/(1000*60*60*24));if(daysSince>=365)senPts+=80;else if(daysSince>=180)senPts+=60;else if(daysSince>=90)senPts+=40;else if(daysSince>=30)senPts+=25;else if(daysSince>=7)senPts+=15;else senPts+=5;}
if(user&&user.derniere_connexion){var lastLogin=new Date(user.derniere_connexion);var now2=new Date();var daysSinceLogin=Math.floor((now2-lastLogin)/(1000*60*60*24));if(daysSinceLogin<=1)senPts+=40;else if(daysSinceLogin<=7)senPts+=30;else if(daysSinceLogin<=30)senPts+=15;}
var history=getScoreHistory();if(history.length>=5)senPts+=30;else if(history.length>=2)senPts+=15;scores.seniority=Math.min(senPts,150);var total=0;DIMENSIONS.forEach(function(d){total+=(scores[d.id]||0);});total=Math.min(total,1000);saveScoreToHistory(total,scores);return{total:total,dimensions:scores};}
function getProfile(){try{return JSON.parse(localStorage.getItem(PROFILE_KEY)||'{}');}catch(e){return{};}}
function getUser(){try{return JSON.parse(localStorage.getItem(USER_KEY)||'null');}catch(e){return null;}}
function getFinancingSearches(){try{return JSON.parse(localStorage.getItem(SEARCHES_KEY)||'[]');}catch(e){return[];}}
function getMarketplaceData(){try{var offers=JSON.parse(localStorage.getItem('agroprix_my_offers')||'[]');var confirmed=offers.filter(function(o){return o.status==='confirmed'||o.status==='sold';});var totalVol=0;confirmed.forEach(function(o){totalVol+=(o.price||0)*(o.quantity||0);});return{offers:offers.length,confirmedSales:confirmed.length,totalVolumeFCFA:totalVol,rating:parseFloat(localStorage.getItem('agroprix_seller_rating')||'0')};}catch(e){return{offers:0,confirmedSales:0,totalVolumeFCFA:0,rating:0};}}
function getScoreHistory(){try{return JSON.parse(localStorage.getItem(SCORE_HISTORY_KEY)||'[]');}catch(e){return[];}}
function saveScoreToHistory(total,dimensions){var history=getScoreHistory();var entry={date:new Date().toISOString().split('T')[0],score:total,dimensions:dimensions};if(history.length>0&&history[history.length-1].date===entry.date){history[history.length-1]=entry;}else{history.push(entry);}
if(history.length>90)history=history.slice(-90);localStorage.setItem(SCORE_HISTORY_KEY,JSON.stringify(history));}
function getBadge(score){for(var i=0;i<BADGES.length;i++){if(score>=BADGES[i].min)return BADGES[i];}
return BADGES[BADGES.length-1];}
function getTips(result){var tips=[];var dims=result.dimensions;if(dims.profile<100){var missing=[];var p=getProfile();if(!p.crops||p.crops.length===0)missing.push('cultures');if(!p.farmSize)missing.push('superficie');if(!p.age)missing.push('age');if(!p.gender)missing.push('genre');if(!p.experience&&p.experience!==0)missing.push('experience');if(missing.length>0){tips.push({icon:'<i data-lucide="user" class="lc"></i>',text:'Completez votre profil ('+missing.join(', ')+') pour gagner jusqu\'a '+(150-dims.profile)+' points.',pts:150-dims.profile});}}
if(dims.transactions<50){tips.push({icon:'<i data-lucide="coins" class="lc"></i>',text:'Publiez des offres sur le Marche et confirmez des ventes pour gagner jusqu\'a 250 points.',pts:250-dims.transactions});}
if(dims.stability<80){var p2=getProfile();if(!p2.isCoop)tips.push({icon:'<i data-lucide="handshake" class="lc"></i>',text:'Rejoignez une cooperative pour +40 points de stabilite.',pts:40});if(!p2.hasBusinessPlan)tips.push({icon:'<i data-lucide="file-text" class="lc"></i>',text:'Preparez un business plan pour +40 points.',pts:40});}
if(dims.usage<50){tips.push({icon:'<i data-lucide="smartphone" class="lc"></i>',text:'Utilisez l\'app regulierement (analyses, recherches financement) pour ameliorer votre score.',pts:100-dims.usage});}
tips.sort(function(a,b){return b.pts-a.pts;});return tips.slice(0,4);}
function renderGauge(score,size){size=size||180;var badge=getBadge(score);var radius=(size/2)-12;var circumference=2*Math.PI*radius;var progress=(score/1000)*circumference;var cx=size/2;var cy=size/2;return'<svg width="'+size+'" height="'+size+'" viewBox="0 0 '+size+' '+size+'">'
...
+'</svg>';}
function renderWidget(){var result=calculate();var badge=getBadge(result.total);return'<div onclick="showView(\'scoring\')" style="cursor:pointer;display:flex;align-items:center;gap:12px;'
