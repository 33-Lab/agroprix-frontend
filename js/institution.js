/*! AgroPrix institution.js - generated from institution.js.src on 2026-06-30 - DO NOT EDIT; edit the .src file and run `python build_js.py` */
...
function applyRole(user){var isInst=user&&user.role==='institution';var nav=document.getElementById('navInstitution');if(nav)nav.style.display=isInst?'':'none';try{document.querySelectorAll('[data-nav="1"]').forEach(function(a){if(a.getAttribute('data-view')!=='institution'){a.style.display=isInst?'none':'';}});document.querySelectorAll('.sidebar-nav .nav-section-label').forEach(function(l){l.style.display=isInst?'none':'';});var bottom=document.getElementById('bottomNav');if(bottom){var more=document.getElementById('moreBtn');if(more)more.style.display=isInst?'none':'';var instBtn=document.getElementById('bottomNavInstitution');if(isInst&&!instBtn){instBtn=document.createElement('a');instBtn.id='bottomNavInstitution';instBtn.className='active';instBtn.setAttribute('data-action','show-view');instBtn.setAttribute('data-view','institution');instBtn.setAttribute('aria-label','Espace institution');instBtn.innerHTML='<span><i data-lucide="landmark" class="lc"></i></span><span>Institution</span>';bottom.insertBefore(instBtn,bottom.firstChild);if(window.lucide){try{window.lucide.createIcons();}catch(e){}}}else if(instBtn){instBtn.style.display=isInst?'':'none';}}}catch(e){}
if(isInst&&window.showView){try{window.showView('institution',nav);}catch(e){}}}
...
function renderError(err){var c=container();if(!c)return;var msg=(err&&err.code===401)?"Connectez-vous avec un compte institution pour accéder à cet espace.":(err&&err.code===403)?"Cet espace est réservé aux comptes institution (banque, microfinance, assureur).":"Cet espace est réservé aux institutions partenaires (banque, microfinance, assureur). Connectez-vous avec un compte institution pour y accéder.";c.innerHTML='<div class="card" style="padding:24px;text-align:center;">'
...
el.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;">'
+'<span style="font-size:12px;color:var(--text-light);">'+list.length+' demandeur(s)</span>'
+'<button data-action="inst-export" style="font-size:11px;font-weight:700;padding:6px 12px;border:1px solid var(--primary);'
+'background:#fff;color:var(--primary);border-radius:8px;cursor:pointer;white-space:nowrap;">'
+'<i data-lucide="download" class="lc" style="width:13px;height:13px;vertical-align:-2px;"></i> Télécharger (CSV)</button>'
+'</div>'
...
AP.institution={init:init,applyRole:applyRole,renderProducerApply:renderProducerApply};AP.actions=AP.actions||{};AP.actions['inst-tab']=function(el){_state.tab=el.getAttribute('data-tab');renderTab();};AP.actions['inst-applicant-detail']=function(el){showApplicantDetail(parseInt(el.getAttribute('data-uid'),10));};AP.actions['inst-status-change']=function(el){setStatus(parseInt(el.getAttribute('data-uid'),10),el.value);};AP.actions['inst-save-criteria']=function(){saveCriteria();};AP.actions['inst-import-financials']=function(){importFinancials();};AP.actions['inst-apply']=function(el){applyToInstitution(el.getAttribute('data-slug'),el);};AP.actions['inst-export']=function(el){downloadApplicantsCsv(el);};function downloadApplicantsCsv(btn){var url=apiBase()+'/api/institution-portal/applicants/export';var orig=btn?btn.innerHTML:'';if(btn){btn.disabled=true;btn.innerHTML='Téléchargement…';}
fetch(url,{credentials:'include'}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.blob();}).then(function(blob){var a=document.createElement('a');var obj=URL.createObjectURL(blob);a.href=obj;a.download='demandeurs.csv';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(obj);}).catch(function(e){alert('Téléchargement impossible : '+(e&&e.message?e.message:e));}).then(function(){if(btn){btn.disabled=false;btn.innerHTML=orig;if(window.lucide)try{window.lucide.createIcons();}catch(e){}}});}})(window.AgroPrix);