...
// ============================================================
// Souscription Plan — Paiement KKiaPay (MTN MoMo, Moov, Carte)
// ============================================================
window.souscrirePlan = function(plan, montant) {
  var AP = window.AgroPrix;
  var user = JSON.parse(localStorage.getItem('agroprix_user') || '{}');
  var kkiapayKey = AP && AP.KKIAPAY_KEY;
  var keyReady = kkiapayKey && kkiapayKey !== 'VOTRE_CLE_PUBLIQUE_KKIAPAY_ICI';

  // Si KKiaPay est configuré → paiement réel
  if (keyReady && window.openKkiapayWidget) {
    // Écouter le succès (une seule fois)
    if (window.addSuccessListener) {
      window.addSuccessListener(function(response) {
        onKkiapaySuccess(plan, montant, response);
      });
    }
    ...
    window.openKkiapayWidget({
      amount: montant,
      api_key: kkiapayKey,
      sandbox: AP.KKIAPAY_SANDBOX || false,
      email: user.email || '',
      phone: user.phone || '',
      name: user.nom || user.name || 'Client AgroPrix',
  ...
  // Fallback simulation (tant que la clé n'est pas configurée)
...
// Callback succès paiement KKiaPay
function onKkiapaySuccess(plan, montant, response) {
  var transactionId = response && response.transactionId ? response.transactionId : 'N/A';
  var user = JSON.parse(localStorage.getItem('agroprix_user') || '{}');

  // Mettre à jour le plan de l'utilisateur en localStorage
  user.plan = plan;
